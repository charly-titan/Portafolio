...
class User extends Eloquent implements UserInterface, RemindableInterface {
	...
	protected function validateUserGoogle(){
		...
		if ($this->isCorporate($user_info["email"])){
			...
			if($this->logUser()){
				Log::notice('El usuario :'.Session::get('user.firstname')." ".Session::get('user.lastname')." - ".Session::get('user.id').' inicio su sesión desde su cuenta Google');
				return  Redirect::to('/');
			}else{
				
				$msg="El Usuario ".$user_info["email"]." desea ingresar a Research pero no cuenta con permisos";
				$datos = array(
	...
	protected function isCorporate($email){
		$email = explode("@", $email);
		if(($email[1]=="televisatim.com")||($email[1]=="esmas.net")||($email[1]=="televisa.com.mx")){
			return true;
		}else{
			return false;
		}
	...
	protected function createSession($profile,$email,$id){
            ...
            Session::put('user.gravatar', md5(strtolower(trim($email))) );
		...
		try
			{
                ...
                if($profile){
                    	$this->createSession($profile,$user_info["email"],$user->id);
                }
