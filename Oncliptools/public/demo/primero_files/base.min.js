...
var Televisa=(function(h,d){var g={},l=false,b=true,e={},k={},f=d.touch,c=window.lastWidthCheck=null,i={SMALL:"small",MEDIUM:"medium",LARGE:"large"},j=i.SMALL;
function a(n){var m=g.getClientWidth();
if(m<624){j=i.SMALL
}else{if(m>=624&&m<960){j=i.MEDIUM
}else{j=i.LARGE
}}h(Televisa).trigger("resize.Televisa",n);
return
}g.throttle=function(m,o){var n;
o=o||500;
return function(){var p=arguments,q=this;
clearTimeout(n);
n=setTimeout(function(){m.apply(q,p)
},o)
}
};
g.init=function(){a();
h(window).resize(function(){if(c!==g.getClientWidth()){c=g.getClientWidth();
a()
}});
l=true;
for(var n in e){e[n].run()
}if(window.ieVersion!==undefined&&window.ieVersion===7){setTimeout(function(){h("body").show()
},1500)
}window.fbAsyncInit=function(){};
(function(m){}(document));
if(g.isIOS()){h('input[type="text"], input[type="number"], textarea, select').bind("focus blur",function(m){if(window.orientation===0){h('meta[name="viewport"]').attr("content","width=device-width, initial-scale=1, maximum-scale="+(m.type==="blur"?10:1))
}})
}return
};
g.registerModule=function(n){e[n.getName()]=n;
return
};
g.moduleReady=function(n){k[n.getName()]=n;
delete e[n.getName()];
return
};
g.getDeviceSize=function(){return j
};
g.getClientWidth=function(){return(document.documentElement||document.body).clientWidth
};
g.getIsTouchDevice=function(){return f
};
g.isIE7=function(n){return(h.browser.msie&&parseInt(h.browser.version,10)===7)
};
g.isKindle=function(){return navigator.userAgent.toLowerCase().indexOf("silk")>-1
};
g.isAndroid=function(){return !!navigator.userAgent.match(/Android/i)
};
g.isIOS=function(){return !!navigator.userAgent.match(/(iPad|iPhone|iPod)/i)
};
g.isBlackBerryCurve=function(){var n=navigator.userAgent;
if(n.indexOf("BlackBerry")>=0){if(n.indexOf("Version/")>=0){var m=n.match(/BlackBerry\s[0-9]*/);
if(m){var o=m[0].match(/[0-9]+/);
if(o){o=o[0]
}pos=n.indexOf("Version/")+8;
os_version=n.substring(pos,pos+3);
if(os_version==="6.0"&&o==="9300"){return true
}}}}return false
};
g.isLessThanIE9=function(n){return(h.browser.msie&&parseInt(h.browser.version,10)<9)
};
h(document).ready(g.init);
return g
...
b<e;
b++){var s=D(a[b].replace(E,h).replace(F,h))+o,l=[];
a[b]=s.replace(G,function(a,b,c,d,e){if(b){if(l.length>0){var a=l,f,e=s.substring(0,e).replace(H,i);
if(e==i||e.charAt(e.length-1)==o){e+="*"
}try{f=t(e)
}catch(k){}if(f){e=0;
for(c=f.length;
e<c;
e++){for(var d=f[e],h=d.className,j=0,m=a.length;
j<m;
j++){var g=a[j];
...
}else{if(b=c?I(c):!v||v.test(d)?{className:w(d),b:!0}:null){return l.push(b),"."+b.className
}return a
}})
}return d+a.join(",")
})
}function I(a){var c=!0,d=w(a.slice(1)),b=a.substring(0,5)==":not(",e,f;
b&&(a=a.slice(5,-1));
var l=a.indexOf("(");
l>-1&&(a=a.substring(0,l));
if(a.charAt(0)==":"){switch(a.slice(1)){case"root":c=function(a){return b?a!=p:a==p
};
break;
case"target":if(m==8){c=function(a){function c(){var d=location.hash,e=d.slice(1);
...
case"checked":c=function(a){J.test(a.type)&&k(a,"propertychange",function(){event.propertyName=="checked"&&g(a,d,a.checked!==b)
});
return a.checked!==b
};
break;
case"disabled":b=!b;
case"enabled":c=function(c){if(K.test(c.tagName)){return k(c,"propertychange",function(){event.propertyName=="$disabled"&&g(c,d,c.a===b)
}),q.push(c),c.a=c.disabled,c.disabled===b
}return a==":enabled"?b:!b
};
break;
case"focus":e="focus",f="blur";
case"hover":e||(e="mouseenter",f="mouseleave");
...
}function g(a,c,d){var b=a.className,c=u(b,c,d);
if(c!=b){a.className=c,a.parentNode.className+=i
}}function u(a,c,d){var b=RegExp("(^|\\s)"+c+"(\\s|$)"),e=b.test(a);
return d?e?a:a+o+c:e?a.replace(b,h).replace(x,h):a
}function k(a,c,d){a.attachEvent("on"+c,d)
}function r(a,c){if(/^https?:\/\//i.test(a)){return c.substring(0,c.indexOf("/",8))==a.substring(0,a.indexOf("/",8))?a:null
}if(a.charAt(0)=="/"){return c.substring(0,c.indexOf("/",8))+a
}var d=c.split(/[?#]/)[0];
a.charAt(0)!="?"&&d.charAt(d.length-1)!="/"&&(d=d.substring(0,d.lastIndexOf("/")+1));
return d+a
}function y(a){if(a){return n.open("GET",a,!1),n.send(),(n.status==200?n.responseText:i).replace(R,i).replace(S,function(c,d,b,e,f){return y(r(b||f,a))
}).replace(T,function(c,d,b){d=d||i;
...
b++){if(c=f.styleSheets[b],c.href!=i&&(a=r(c.href,d))){c.cssText=A(y(a))
}}q.length>0&&setInterval(function(){for(var a=0,c=q.length;
a<c;
a++){var b=q[a];
...
(function(a,c){function d(){try{p.doScroll("left")
}catch(a){setTimeout(d,50);
return
}b("poll")
...
}else{if(f.createEventObject&&p.doScroll){try{g=!a.frameElement
}catch(h){}g&&d()
}k(f,"readystatechange",b);
k(a,"load",b)
}})(j,function(){for(var a in z){var c,d,b=j;
if(j[a]){for(c=z[a].replace("*",a).split(".");
(d=c.shift())&&(b=b[d]);
){}if(typeof b=="function"){t=b;
U();
break
}}}})
}}})(this);
...
if(f in w.style){return f
}for(f=0;
f<e.length;
++f){var g=e[f]+i;
if(g in w.style){return g
}}}function v(d){"string"===typeof d&&this.parse(d);
return this
}function c(e,d,f){!0===d?e.queue(f):d?e.queue(d,f):f()
}function t(e){var d=[];
A.each(e,function(f){f=A.camelCase(f);
f=A.transit.propertyMap[f]||f;
f=a(f);
-1===A.inArray(f,d)&&d.push(f)
});
return d
}function b(g,d,m,k){g=t(g);
A.cssEase[m]&&(m=A.cssEase[m]);
var i=""+l(d)+" "+m;
0<parseInt(k,10)&&(i+=" "+l(k));
var j=[];
A.each(g,function(f,e){j.push(e+" "+i)
});
return j.join(", ")
}function y(e,d){d||(A.cssNumber[e]=!0);
A.transit.propertyMap[e]=z.transform;
A.cssHooks[e]={get:function(f){return(A(f).css("transform")||new v).get(e)
},set:function(f,i){var g=A(f).css("transform")||new v;
g.setFromString(e,i);
A(f).css({transform:g})
}}
}function a(d){return d.replace(/([A-Z])/g,function(e){return"-"+e.toLowerCase()
})
}function x(e,d){return"string"===typeof e&&!e.match(/^[\-0-9\.]+$/)?e:""+e+d
}function l(d){A.fx.speeds[d]&&(d=A.fx.speeds[d]);
return x(d,"ms")
...
var w=document.createElement("div"),z={},B=-1<navigator.userAgent.toLowerCase().indexOf("chrome");
...
var h=z.transitionEnd={MozTransition:"transitionend",OTransition:"oTransitionEnd",WebkitTransition:"webkitTransitionEnd",msTransition:"MSTransitionEnd"}[z.transition]||null,w=null;
A.cssEase={_default:"ease","in":"ease-in",out:"ease-out","in-out":"ease-in-out",snap:"cubic-bezier(0,1,.5,1)"};
A.cssHooks.transform={get:function(d){return A(d).data("transform")
},set:function(e,d){var f=d;
f instanceof v||(f=new v(f));
e.style[z.transform]="WebkitTransform"===z.transform&&!B?f.toString(!0):f.toString();
A(e).data("transform",f)
}};
A.cssHooks.transformOrigin={get:function(d){return d.style[z.transformOrigin]
},set:function(e,d){e.style[z.transformOrigin]=d
}};
A.cssHooks.transition={get:function(d){return d.style[z.transition]
},set:function(e,d){e.style[z.transition]=d
}};
y("scale");
y("translate");
y("rotate");
...
y("x",!0);
y("y",!0);
v.prototype={setFromString:function(e,d){var f="string"===typeof d?d.split(","):d.constructor===Array?d:[d];
f.unshift(e);
v.prototype.set.apply(this,f)
},set:function(e){var d=Array.prototype.slice.apply(arguments,[1]);
this.setter[e]?this.setter[e].apply(this,d):this[e]=d.join(",")
},get:function(d){return this.getter[d]?this.getter[d].apply(this):this[d]||0
},setter:{rotate:function(d){this.rotate=x(d,"deg")
},rotateX:function(d){this.rotateX=x(d,"deg")
},rotateY:function(d){this.rotateY=x(d,"deg")
},scale:function(e,d){void 0===d&&(d=e);
this.scale=e+","+d
},skewX:function(d){this.skewX=x(d,"deg")
},skewY:function(d){this.skewY=x(d,"deg")
},perspective:function(d){this.perspective=x(d,"px")
},x:function(d){this.set("translate",d,null)
},y:function(d){this.set("translate",null,d)
},translate:function(e,d){void 0===this._translateX&&(this._translateX=0);
...
},scale:function(){var d=(this.scale||"1,1").split(",");
d[0]&&(d[0]=parseFloat(d[0]));
d[1]&&(d[1]=parseFloat(d[1]));
return d[0]===d[1]?d[0]:d
},rotate3d:function(){for(var e=(this.rotate3d||"0,0,0,0deg").split(","),d=0;
3>=d;
++d){e[d]&&(e[d]=parseFloat(e[d]))
}e[3]&&(e[3]=x(e[3],"deg"));
return e
}},parse:function(e){var d=this;
e.replace(/([a-zA-Z0-9]+)\((.*?)\)/g,function(f,i,g){d.setFromString(i,g)
})
},toString:function(e){var d=[],f;
...
A.fn.transition=A.fn.transit=function(E,D,C,s){var q=this,r=0,p=!0;
...
var D=l(D),o=b(E,D,C,r),e=A.transit.enabled&&z.transition?parseInt(D,10)+parseInt(r,10):0;
if(0===e){return c(q,p,function(f){q.css(E);
s&&s.apply(q);
f&&f()
}),q
}var n={},d=function(f){var j=false,i=function(){j&&q.unbind(h,i);
e>0&&q.each(function(){this.style[z.transition]=n[this]||null
});
typeof s==="function"&&s.apply(q);
typeof f==="function"&&f()
};
if(e>0&&h&&A.transit.useTransitionEnd){j=true;
q.bind(h,i)
}else{window.setTimeout(i,e)
}q.each(function(){e>0&&(this.style[z.transition]=o);
A(this).css(E)
})
};
c(q,p,function(g){var f=0;
z.transition==="MozTransition"&&f<25&&(f=25);
...
window.Swipe=function(b,a){if(!b){return null
}var c=this;
this.options=a||{};
...
this.setup();
this.begin();
if(this.element.addEventListener){this.element.addEventListener("touchstart",this,false);
...
Swipe.prototype={setup:function(){this.slides=this.element.children;
this.length=this.slides.length;
if(this.length<2){return null
}this.width=Math.ceil(("getBoundingClientRect" in this.container)?this.container.getBoundingClientRect().width:this.container.offsetWidth);
if(this.width===0&&typeof window.getComputedStyle==="function"){this.width=window.getComputedStyle(this.container,null).width.replace("px","")
}if(!this.width){return null
}var b=this.container.style.visibility;
this.container.style.visibility="hidden";
this.element.style.width=Math.ceil(this.slides.length*this.width)+"px";
var a=this.slides.length;
while(a--){var c=this.slides[a];
c.style.width=this.width+"px";
c.style.display="table-cell";
c.style.verticalAlign="top"
}this.slide(this.index,0);
this.container.style.visibility=b
},slide:function(a,c){var b=this.element.style;
if(c==undefined){c=this.speed
...
},prev:function(a){this.delay=a||0;
...
}},begin:function(){var a=this;
this.interval=(this.delay)?setTimeout(function(){a.next(a.delay)
},this.delay):0
},stop:function(){this.delay=0;
clearTimeout(this.interval)
},resume:function(){this.delay=this.options.auto||0;
this.begin()
},handleEvent:function(a){switch(a.type){case"touchstart":this.onTouchStart(a);
break;
case"touchmove":this.onTouchMove(a);
break;
case"touchcancel":case"touchend":this.onTouchEnd(a);
break;
...
break
}},transitionEnd:function(a){if(this.delay){this.begin()
...
}if(!this.isScrolling){a.preventDefault();
clearTimeout(this.interval);
this.deltaX=this.deltaX/((!this.index&&this.deltaX>0||this.index==this.length-1&&this.deltaX<0)?(Math.abs(this.deltaX)/this.width+1):1);
this.element.style.MozTransform=this.element.style.webkitTransform="translate3d("+(this.deltaX-this.index*this.width)+"px,0,0)";
a.stopPropagation()
}},onTouchEnd:function(c){var b=Number(new Date())-this.start.time<250&&Math.abs(this.deltaX)>20||Math.abs(this.deltaX)>this.width/2,a=!this.index&&this.deltaX>0||this.index==this.length-1&&this.deltaX<0;
if(!this.isScrolling){this.slide(this.index+(b&&!a?(this.deltaX<0?1:-1):0),this.speed)
}c.stopPropagation()
}};
$(document).ready(function(){(function(c,b){function d(l){var m={},k=null,h=l.find(".galleryThumbs"),i=l.find(".galleryImages>ul");
if(!h.length||!i.length){return
}var f=h.find("li").map(function(n,o){return c(o).attr("data-assetid")
}).get();
a(f).done(function(s){var q=i.find("li:first-child"),t=q.attr("data-assetid"),p=c.grep(s,function(v,u){return t==v.id
});
g(q,p[0],true);
var n=c.grep(s,function(v,u){return t!=v.id
});
c.each(n,function(u,w){var v=c("<li></li>");
v.data("galleryElement",w);
i.append(v);
g(v,w)
});
if(Modernizr.cssanimations&&Modernizr.touch){var r=l.find(".galleryImages").get(0);
k=new Swipe(r,{callback:o})
}function o(){var u=k.getPos();
m[u]()
}j()
});
function e(q){var p=h.find(">li.active"),n=h.find(">li"),o=n.index(p);
if((o>0&&q==-1)||(o<n.length-1&&q==1)){o+=q;
c(h.find(">li").get(o)).find(">a").click()
}}function j(){if(/iPhone|iPod|iPad/.test(navigator.userAgent)&&!(/OS [2-4]_\d(_\d)? like Mac OS X/i.test(navigator.userAgent))){}else{if(b.getIsTouchDevice()&&!b.isBlackBerryCurve()){}else{l.addClass("showArrows")
}}l.find(".title ul .left").click(function(n){n.preventDefault();
if(k){k.prev()
}else{e(-1)
}});
l.find(".title ul .right").click(function(n){n.preventDefault();
if(k){k.next()
}else{e(1)
}})
}function g(u,q,s){var o=false,r=h.find('>li[data-assetid="'+q.id+'"]'),t=r.find(">a"),p=h.find(">li"),n=p.index(r),v=function(){l.find(".itemSection a").attr("href",q.urlSection).html(q.section);
l.find(".itemCaption a").attr("href",t.attr("href")).html(q.caption);
l.find(".galleryCaption h3").html(q.age);
p.removeClass("active");
r.addClass("active");
if(!s&&!o){var x=c('<a href="'+t.attr("href")+'"></a>');
var w='<img src="'+q.imgUrl+'" />';
x.append(w);
u.append(x);
o=true
}};
m[n]=v;
t.data("galleryElement",q).click(function(w){w.preventDefault();
if(k){k.slide(n,0)
}else{i.find("li.active").removeClass("active");
u.addClass("active")
}v()
})
}}function a(f){var e=c.Deferred();
c.getJSON("../global/json/articlesdata.json?assets="+encodeURIComponent(f.join(","))).done(function(g){e.resolve(g)
});
return e
}c(".verticalHero").each(function(f,e){d(c(e))
})
})(jQuery,Televisa)
});
$(document).ready(function(){(function(d,a){var c=d(".type1a div:last-child"),b=500;
c.each(function(){var i=d(this).find("ul li"),h=(d(this).parent().hasClass("type1c"))?41:0,f={UP:(a.getIsTouchDevice())?"touchend":"mouseup",DOWN:(a.getIsTouchDevice())?"touchstart":"click"},g=d(this);
if(/Android|iPhone|iPod|iPad/.test(navigator.userAgent)&&!(/OS [2-4]_\d(_\d)? like Mac OS X/i.test(navigator.userAgent))){d(this).addClass("iOS")
}else{var e=/Windows Phone/.test(navigator.userAgent);
if(a.getIsTouchDevice()&&!a.isBlackBerryCurve()){d(this).bind("touchstart",function(k){var j=k.originalEvent.changedTouches[0].clientX;
d(this).attr("startX",j);
d(this).attr("down",true);
d(this).attr("scrollLeft",d(this).scrollLeft())
});
d(this).bind("touchend",function(j){if(d(this).attr("active")==="true"){j.preventDefault();
j.stopPropagation()
}d(this).attr("active",false);
d(this).attr("down",false)
});
d(this).bind("touchmove",function(l){var k=l.originalEvent.changedTouches[0].clientX,m=d(this).attr("startX")>>0,j=k;
if(Math.abs(m-j)>10){d(this).attr("active",true)
}if(d(this).attr("active")==="true"){l.preventDefault();
d(this).scrollLeft((d(this).attr("scrollLeft")>>0)+(m-j))
}return false
})
}else{d(this).parent().addClass("showArrows")
}}g.parent().find("a.left").click(function(j){j.preventDefault();
g.animate({scrollLeft:g.scrollLeft()-g.width()},b)
});
g.parent().find("a.right").click(function(j){j.preventDefault();
g.animate({scrollLeft:g.scrollLeft()+g.width()},b)
})
})
}(jQuery,Televisa))
});
$(document).ready(function(d,a){var c=d(".type1a div:last-child"),b=500;
c.each(function(){var i=d(this).find("ul li"),h=(d(this).parent().hasClass("type1c"))?41:24,f={UP:(a.getIsTouchDevice())?"touchend":"mouseup",DOWN:(a.getIsTouchDevice())?"touchstart":"mousedown"},g=d(this);
d(this).find("ul").width((i.outerWidth(true)+h)*i.length-144);
if(/Android|iPhone|iPod|iPad/.test(navigator.userAgent)&&!(/OS [2-4]_\d(_\d)? like Mac OS X/i.test(navigator.userAgent))){d(this).addClass("iOS")
}else{var e=/Windows Phone/.test(navigator.userAgent);
if(a.getIsTouchDevice()&&!a.isBlackBerryCurve()){if(!d(this).parent().hasClass("type1b")){d(this).parent().find("div:first-child ul").hide()
}d(this).bind("touchstart",function(k){var j=k.originalEvent.changedTouches[0].clientX;
d(this).attr("startX",j);
d(this).attr("down",true);
d(this).attr("scrollLeft",d(this).scrollLeft())
});
...
g.animate({scrollLeft:g.scrollLeft()-340},b)
});
g.parent().find("a.right").click(function(j){j.preventDefault();
g.animate({scrollLeft:g.scrollLeft()+340},b)
})
...
$(document).ready(function(){(function(d,a){var c=d(".wdg_carousa > div:last-child"),b=500;
c.each(function(){var i=d(this).find("ul li"),h=(d(this).parent().hasClass("wdg_carousa"))?28:41,f={UP:(a.getIsTouchDevice())?"touchend":"mouseup",DOWN:(a.getIsTouchDevice())?"touchstart":"mousedown"},g=d(this);
d(this).find("ul").width((i.outerWidth(true)+h)*i.length-h);
...
d(this).bind("touchmove",function(r){var k=r.originalEvent.changedTouches[0].clientX,t=d(this).attr("startX")>>0,s=k;
if(Math.abs(t-s)>10){d(this).attr("active",true)
}if(d(this).attr("active")==="true"){r.preventDefault();
d(this).scrollLeft((d(this).attr("scrollLeft")>>0)+(t-s))
}var l=d(window).width();
if(l<=960){var m=g.parent("div").parent("div").find(".bullets ul li");
var p=0;
if(l<=624){p=g.find("ul").width()-l
}else{p=g.find("ul").width()-300
}var n=p/5;
for(var o=1;
o<=m.length;
o++){var q=(n*o)-n;
var j=n*o;
if(parseInt(g.attr("scrollleft"))>=q&&parseInt(g.attr("scrollleft"))<j){d(m).removeClass("selected");
d(m[o-1]).addClass("selected")
}}}return false
})
}else{d(this).parent().addClass("showArrows")
}g.parent().find("a.wdg_carousa_left").click(function(j){j.preventDefault();
g.animate({scrollLeft:g.scrollLeft()-328},b)
});
g.parent().find("a.wdg_carousa_right").click(function(j){j.preventDefault();
g.animate({scrollLeft:g.scrollLeft()+328},b)
})
})
}(jQuery,Televisa))
...
d.fn.swipe=function(A){var z=d(this),y=z.data(w);
if(y&&typeof A==="string"){if(y[A]){return y[A].apply(this,Array.prototype.slice.call(arguments,1))
}else{d.error("Method "+A+" does not exist on jQuery.swipe")
}}else{if(!y&&(typeof A==="object"||!A)){return t.apply(this,arguments)
}}return z
};
...
function t(y){if(y&&(y.allowPageScroll===undefined&&(y.swipe!==undefined||y.swipeStatus!==undefined))){y.allowPageScroll=k
}if(!y){y={}
}y=d.extend({},d.fn.swipe.defaults,y);
return this.each(function(){var A=d(this);
var z=A.data(w);
if(!z){z=new p(this,y);
A.data(w,z)
}})
}function p(S,af){var aF=(a||!af.fallbackToMouseEvents),ax=aF?"touchstart":"mousedown",U=aF?"touchmove":"mousemove",au=aF?"touchend":"mouseup",D=aF?null:"mouseleave",R="touchcancel";
var ac=0;
var N=null;
var ag=0;
var aB=0;
...
try{H.bind(ax,ar);
H.bind(R,M)
}catch(aC){d.error("events not supported "+ax+","+R+" on jQuery.swipe")
}this.enable=function(){H.bind(ax,ar);
H.bind(R,M);
return H
};
...
function ar(aM){if(X()){return
}if(d(aM.target).closest(af.excludedElements,H).length>0){return
}var aN=aM.originalEvent;
var aL,aK=a?aN.touches[0]:aN;
O=f;
if(a){aE=aN.touches.length
...
ah=T();
z();
if(!a||(aE===af.fingers||af.fingers===h)||ao()){aI(0,aK);
I=B();
if(aE==2){aI(1,aN.touches[1]);
aB=A=Z(ah[0].start,ah[1].start)
}if(af.swipeStatus||af.pinchStatus){aL=aD(aN,O)
}}else{aL=false
}if(aL===false){O=o;
aD(aN,O);
return aL
}else{aj(true)
}}function P(aN){var aQ=aN.originalEvent;
if(O===g||O===o||ae()){return
}var aM,aL=a?aQ.touches[0]:aQ;
var aO=V(aL);
Y=B();
if(a){aE=aQ.touches.length
}O=i;
if(aE==2){if(aB==0){aI(1,aQ.touches[1]);
aB=A=Z(ah[0].start,ah[1].start)
}else{V(aQ.touches[1]);
...
}if((aE===af.fingers||af.fingers===h)||!a||ao()){N=aq(aO.start,aO.end);
C(aN,N);
ac=G(aO.start,aO.end);
ag=L();
if(af.swipeStatus||af.pinchStatus){aM=aD(aQ,O)
}if(!af.triggerOnTouchEnd||af.triggerOnTouchLeave){var aK=true;
if(af.triggerOnTouchLeave){var aP=at(this);
aK=az(aO.end,aP)
}if(!af.triggerOnTouchEnd&&aK){O=aG(i)
}else{if(af.triggerOnTouchLeave&&!aK){O=aG(g)
}}if(O==o||O==g){aD(aQ,O)
}}}else{O=o;
aD(aQ,O)
}if(aM===false){O=o;
aD(aQ,O)
}}function aa(aM){var aO=aM.originalEvent;
if(a){if(aO.touches.length>0){av();
return true
}}if(ae()){aE=aJ
}aM.preventDefault();
Y=B();
if(af.triggerOnTouchEnd||(af.triggerOnTouchEnd==false&&O===i)){O=g;
...
z();
aj(false)
}function W(aK){var aL=aK.originalEvent;
if(af.triggerOnTouchLeave){O=aG(g);
aD(aL,O)
}}function Q(){H.unbind(ax,ar);
H.unbind(R,M);
H.unbind(U,P);
H.unbind(au,aa);
if(D){H.unbind(D,W)
}aj(false)
}function aG(aN){var aM=aN;
var aL=ap();
var aK=ad();
if(!aL){aM=o
}else{if(aK&&aN==i&&(!af.triggerOnTouchEnd||af.triggerOnTouchLeave)){aM=g
}else{if(!aK&&aN==g&&af.triggerOnTouchLeave){aM=o
}}}return aM
}function aD(aM,aK){var aL=undefined;
if(ab()){aL=al(aM,aK,j)
}if(ao()&&aL!==false){aL=al(aM,aK,r)
}if(K()&&aL!==false){aL=al(aM,aK,e)
}if(aK===o){M(aM)
}if(aK===g){if(a){if(aM.touches.length==0){M(aM)
}}else{M(aM)
}}return aL
}function al(aN,aK,aM){var aL=undefined;
if(aM==j){if(af.swipeStatus){aL=af.swipeStatus.call(H,aN,aK,N||null,ac||0,ag||0,aE);
if(aL===false){return false
}}if(aK==g&&ay()){if(af.swipe){aL=af.swipe.call(H,aN,N,ac,ag,aE);
if(aL===false){return false
}}switch(N){case n:if(af.swipeLeft){aL=af.swipeLeft.call(H,aN,N,ac,ag,aE)
}break;
case m:if(af.swipeRight){aL=af.swipeRight.call(H,aN,N,ac,ag,aE)
}break;
case c:if(af.swipeUp){aL=af.swipeUp.call(H,aN,N,ac,ag,aE)
}break;
case u:if(af.swipeDown){aL=af.swipeDown.call(H,aN,N,ac,ag,aE)
}break
}}}if(aM==r){if(af.pinchStatus){aL=af.pinchStatus.call(H,aN,aK,aH||null,pinchDistance||0,ag||0,aE,ai);
if(aL===false){return false
}}if(aK==g&&am()){switch(aH){case b:if(af.pinchIn){aL=af.pinchIn.call(H,aN,aH||null,pinchDistance||0,ag||0,aE,ai)
}break;
case v:if(af.pinchOut){aL=af.pinchOut.call(H,aN,aH||null,pinchDistance||0,ag||0,aE,ai)
}break
}}}if(aM==e){if(aK===o){if(af.click&&(aE===1||!a)&&(isNaN(ac)||ac===0)){aL=af.click.call(H,aN,aN.target)
}}}return aL
}function ad(){if(af.threshold!==null){return ac>=af.threshold
}return true
}function ak(){if(af.pinchThreshold!==null){return pinchDistance>=af.pinchThreshold
}return true
}function ap(){var aK;
if(af.maxTimeThreshold){if(ag>=af.maxTimeThreshold){aK=false
}else{aK=true
}}else{aK=true
}return aK
}function C(aK,aL){if(af.allowPageScroll===k||ao()){aK.preventDefault()
}else{var aM=af.allowPageScroll===q;
switch(aL){case n:if((af.swipeLeft&&aM)||(!aM&&af.allowPageScroll!=x)){aK.preventDefault()
}break;
case m:if((af.swipeRight&&aM)||(!aM&&af.allowPageScroll!=x)){aK.preventDefault()
}break;
case c:if((af.swipeUp&&aM)||(!aM&&af.allowPageScroll!=s)){aK.preventDefault()
}break;
case u:if((af.swipeDown&&aM)||(!aM&&af.allowPageScroll!=s)){aK.preventDefault()
}break
}}}function am(){return ak()
}function ao(){return !!(af.pinchStatus||af.pinchIn||af.pinchOut)
}function aw(){return !!(am()&&ao())
}function ay(){var aK=ap();
var aM=ad();
var aL=aM&&aK;
return aL
}function ab(){return !!(af.swipe||af.swipeStatus||af.swipeLeft||af.swipeRight||af.swipeUp||af.swipeDown)
}function E(){return !!(ay()&&ab())
}function K(){return !!(af.click)
}function av(){aA=B();
aJ=event.touches.length+1
}function z(){aA=0;
aJ=0
}function ae(){var aK=false;
if(aA){var aL=B()-aA;
if(aL<=af.fingerReleaseThreshold){aK=true
}}return aK
}function X(){return !!(H.data(w+"_intouch")===true)
}function aj(aK){if(aK===true){H.bind(U,P);
H.bind(au,aa);
if(D){H.bind(D,W)
}}else{H.unbind(U,P,false);
H.unbind(au,aa,false);
if(D){H.unbind(D,W,false)
}}H.data(w+"_intouch",aK===true)
}function aI(aL,aK){var aM=aK.identifier!==undefined?aK.identifier:0;
ah[aL].identifier=aM;
ah[aL].start.x=ah[aL].end.x=aK.pageX||aK.clientX;
ah[aL].start.y=ah[aL].end.y=aK.pageY||aK.clientY;
return ah[aL]
}function V(aK){var aM=aK.identifier!==undefined?aK.identifier:0;
var aL=J(aM);
aL.end.x=aK.pageX||aK.clientX;
aL.end.y=aK.pageY||aK.clientY;
return aL
}function J(aL){for(var aK=0;
aK<ah.length;
aK++){if(ah[aK].identifier==aL){return ah[aK]
}}}function T(){var aK=[];
for(var aL=0;
aL<=5;
aL++){aK.push({start:{x:0,y:0},end:{x:0,y:0},identifier:0})
}return aK
}function L(){return Y-I
}function Z(aN,aM){var aL=Math.abs(aN.x-aM.x);
var aK=Math.abs(aN.y-aM.y);
return Math.round(Math.sqrt(aL*aL+aK*aK))
}function y(aK,aL){var aM=(aL/aK)*1;
return aM.toFixed(2)
}function an(){if(ai<1){return v
}else{return b
}}function G(aL,aK){return Math.round(Math.sqrt(Math.pow(aK.x-aL.x,2)+Math.pow(aK.y-aL.y,2)))
}function F(aN,aL){var aK=aN.x-aL.x;
var aP=aL.y-aN.y;
var aM=Math.atan2(aP,aK);
var aO=Math.round(aM*180/Math.PI);
if(aO<0){aO=360-Math.abs(aO)
}return aO
}function aq(aL,aK){var aM=F(aL,aK);
if((aM<=45)&&(aM>=0)){return n
}else{if((aM<=360)&&(aM>=315)){return n
}else{if((aM>=135)&&(aM<=225)){return m
}else{if((aM>45)&&(aM<135)){return u
}else{return c
}}}}}function B(){var aK=new Date();
return aK.getTime()
}function at(aK){aK=d(aK);
var aM=aK.offset();
var aL={left:aM.left,right:aM.left+aK.outerWidth(),top:aM.top,bottom:aM.top+aK.outerHeight()};
return aL
}function az(aK,aL){return(aK.x>aL.left&&aK.x<aL.right&&aK.y>aL.top&&aK.y<aL.bottom)
}}})(jQuery);
$(document).ready(function(){$(window).load(function(){$('img[src*="vistas/spacer.gif"]').hide();
$("#social_mailing").hide()
})
});
/*!
...
$(document).ready(function(){(function(d,a){var c=d(".type1a div:last-child"),b=500;
c.each(function(){var j=d(this).find("ul li"),i=(d(this).parent().hasClass("type1c"))?41:0,g={UP:(a.getIsTouchDevice())?"touchend":"mouseup",DOWN:(a.getIsTouchDevice())?"touchstart":"click"},h=d(this);
if(/Android|iPhone|iPod|iPad/.test(navigator.userAgent)&&!(/OS [2-4]_\d(_\d)? like Mac OS X/i.test(navigator.userAgent))){d(this).addClass("iOS")
}else{var f=/Windows Phone/.test(navigator.userAgent);
if(a.getIsTouchDevice()&&!a.isBlackBerryCurve()){d(this).bind("touchstart",function(l){var k=l.originalEvent.changedTouches[0].clientX;
d(this).attr("startX",k);
d(this).attr("down",true);
d(this).attr("scrollLeft",d(this).scrollLeft())
});
d(this).bind("touchend",function(k){if(d(this).attr("active")==="true"){k.preventDefault();
k.stopPropagation()
}d(this).attr("active",false);
d(this).attr("down",false)
});
d(this).bind("touchmove",function(m){var l=m.originalEvent.changedTouches[0].clientX,n=d(this).attr("startX")>>0,k=l;
if(Math.abs(n-k)>10){d(this).attr("active",true)
}if(d(this).attr("active")==="true"){m.preventDefault();
d(this).scrollLeft((d(this).attr("scrollLeft")>>0)+(n-k))
}return false
})
}else{d(this).parent().addClass("showArrows")
}}var e=0;
if(d(this).parent("div").parent("div").hasClass("btn_6prog_02")){e=25
}h.parent().find("a.left").click(function(k){k.preventDefault();
h.animate({scrollLeft:h.scrollLeft()-(h.width()+e)},b)
});
h.parent().find("a.right").click(function(k){k.preventDefault();
h.animate({scrollLeft:h.scrollLeft()+(h.width()+e)},b)
})
})
}(jQuery,Televisa))
...
}})(function(s,j){var c=6,e=s.event.add,v=s.event.remove,y=function(R,Q,S){s.event.trigger(Q,S,R)
...
})(),g={textarea:true,input:true,select:true,button:true},i={move:"mousemove",cancel:"mouseup dragstart",end:"mouseup"},d={move:"touchmove",cancel:"touchend",end:"touchend"};
function n(S){var U=S,T=false,Q=false;
function R(V){if(T){U();
C(R);
Q=true;
T=false
}else{Q=false
}}this.kick=function(V){T=true;
if(!Q){R()
}};
this.end=function(W){var V=U;
if(!W){return
}if(!Q){W()
}else{U=T?function(){V();
W()
...
}function N(Q){if(g[Q.target.tagName.toLowerCase()]){return
}Q.preventDefault()
}function F(Q){return(Q.which===1&&!Q.ctrlKey&&!Q.altKey)
}function L(S,T){var R,Q;
if(S.identifiedTouch){return S.identifiedTouch(T)
}R=-1;
Q=S.length;
while(++R<Q){if(S[R].identifier===T){return S[R]
}}}function l(R,Q){var S=L(R.changedTouches,Q.identifier);
if(!S){return
}if(S.pageX===Q.pageX&&S.pageY===Q.pageY){return
}return S
}function o(R){var Q;
if(!F(R)){return
}Q={target:R.target,startX:R.pageX,startY:R.pageY,timeStamp:R.timeStamp};
e(document,i.move,u,Q);
e(document,i.cancel,x,Q)
}function u(R){var Q=R.data;
r(R,Q,R,z)
}function x(Q){z()
}function z(){v(document,i.move,u);
v(document,i.cancel,x)
}function O(R){var S,Q;
if(g[R.target.tagName.toLowerCase()]){return
}S=R.changedTouches[0];
Q={target:S.target,startX:S.pageX,startY:S.pageY,timeStamp:R.timeStamp,identifier:S.identifier};
e(document,d.move+"."+S.identifier,p,Q);
e(document,d.cancel+"."+S.identifier,D,Q)
}function p(R){var Q=R.data,S=l(R,Q);
if(!S){return
}r(R,Q,S,M)
}function D(R){var Q=R.data,S=L(R.changedTouches,Q.identifier);
if(!S){return
}M(Q.identifier)
}function M(Q){v(document,"."+Q,p);
v(document,"."+Q,D)
}function r(U,T,V,S){var R=V.pageX-T.startX,Q=V.pageY-T.startY;
if((R*R)+(Q*Q)<(c*c)){return
}q(U,T,V,R,Q,S)
}function H(){this._handled=K;
return false
}function J(Q){Q._handled()
}function q(U,Y,S,X,V,W){var R=Y.target,T,Q;
T=U.targetTouches;
Q=U.timeStamp-Y.timeStamp;
Y.type="movestart";
Y.distX=X;
Y.distY=V;
Y.deltaX=X;
...
Y.velocityY=V/Q;
Y.targetTouches=T;
Y.finger=T?T.length:1;
Y._handled=H;
Y._preventTouchmoveDefault=function(){U.preventDefault()
};
y(Y.target,Y);
W(Y.identifier)
}function G(R){var Q=R.data.event,S=R.data.timer;
E(Q,R,R.timeStamp,S)
}function b(R){var Q=R.data.event,S=R.data.timer;
a();
w(Q,S,function(){setTimeout(function(){v(Q.target,"click",P)
},0)
})
}function a(Q){v(document,i.move,G);
v(document,i.end,b)
}function A(R){var Q=R.data.event,T=R.data.timer,S=l(R,Q);
if(!S){return
}R.preventDefault();
Q.targetTouches=R.targetTouches;
E(Q,S,R.timeStamp,T)
}function h(R){var Q=R.data.event,T=R.data.timer,S=L(R.changedTouches,Q.identifier);
if(!S){return
}k(Q);
w(Q,T)
}function k(Q){v(document,"."+Q.identifier,A);
v(document,"."+Q.identifier,h)
}function E(R,U,Q,T){var S=Q-R.timeStamp;
R.type="move";
R.distX=U.pageX-R.startX;
R.distY=U.pageY-R.startY;
R.deltaX=U.pageX-R.pageX;
R.deltaY=U.pageY-R.pageY;
R.velocityX=0.3*R.velocityX+0.7*R.deltaX/S;
R.velocityY=0.3*R.velocityY+0.7*R.deltaY/S;
R.pageX=U.pageX;
R.pageY=U.pageY;
T.kick()
}function w(R,S,Q){S.end(function(){R.type="moveend";
y(R.target,R);
return Q&&Q()
})
}function f(S,R,Q){e(this,"movestart.move",J);
return true
}function t(Q){v(this,"dragstart drag",B);
v(this,"mousedown touchstart",N);
v(this,"movestart",J);
return true
}function I(Q){if(Q.namespace==="move"||Q.namespace==="moveend"){return
}e(this,"dragstart."+Q.guid+" drag."+Q.guid,B,j,Q.selector);
e(this,"mousedown."+Q.guid,N,j,Q.selector)
}function m(Q){if(Q.namespace==="move"||Q.namespace==="moveend"){return
}v(this,"dragstart."+Q.guid+" drag."+Q.guid);
v(this,"mousedown."+Q.guid)
}s.event.special.movestart={setup:f,teardown:t,add:I,remove:m,_default:function(S){var Q,R;
if(!S._handled()){return
}Q={target:S.target,startX:S.startX,startY:S.startY,pageX:S.pageX,pageY:S.pageY,distX:S.distX,distY:S.distY,deltaX:S.deltaX,deltaY:S.deltaY,velocityX:S.velocityX,velocityY:S.velocityY,timeStamp:S.timeStamp,identifier:S.identifier,targetTouches:S.targetTouches,finger:S.finger};
R={event:Q,timer:new n(function(T){y(S.target,Q)
})};
if(S.identifier===j){e(S.target,"click",P);
e(document,i.move,G,R);
e(document,i.end,b,R)
}else{S._preventTouchmoveDefault();
e(document,d.move+"."+S.identifier,A,R);
e(document,d.end+"."+S.identifier,h,R)
}}};
s.event.special.move={setup:function(){e(this,"movestart.move",s.noop)
},teardown:function(){v(this,"movestart.move",s.noop)
}};
s.event.special.moveend={setup:function(){e(this,"movestart.moveend",s.noop)
},teardown:function(){v(this,"movestart.moveend",s.noop)
}};
e(document,"mousedown.move",o);
e(document,"touchstart.move",O);
if(typeof Array.prototype.indexOf==="function"){(function(S,T){var R=["changedTouches","targetTouches"],Q=R.length;
while(Q--){if(S.event.props.indexOf(R[Q])===-1){S.event.props.push(R[Q])
}}})(s)
}});
(function(a){if(typeof define==="function"&&define.amd){define(["jquery"],a)
}else{a(jQuery)
}})(function(g,h){var f=g.event.add,c=g.event.remove,d=function(j,i,k){g.event.trigger(i,k,j)
},e={threshold:0.4,sensitivity:6};
function b(l){var i,j,k;
i=l.target.offsetWidth;
j=l.target.offsetHeight;
k={distX:l.distX,distY:l.distY,velocityX:l.velocityX,velocityY:l.velocityY,finger:l.finger};
if(l.distX>l.distY){if(l.distX>-l.distY){if(l.distX/i>e.threshold||l.velocityX*l.distX/i*e.sensitivity>1){k.type="swiperight";
d(l.currentTarget,k)
}}else{if(-l.distY/j>e.threshold||l.velocityY*l.distY/i*e.sensitivity>1){k.type="swipeup";
d(l.currentTarget,k)
}}}else{if(l.distX>-l.distY){if(l.distY/j>e.threshold||l.velocityY*l.distY/i*e.sensitivity>1){k.type="swipedown";
d(l.currentTarget,k)
}}else{if(-l.distX/i>e.threshold||l.velocityX*l.distX/i*e.sensitivity>1){k.type="swipeleft";
d(l.currentTarget,k)
}}}}function a(i){var j=g.data(i,"event_swipe");
if(!j){j={count:0};
g.data(i,"event_swipe",j)
}return j
}g.event.special.swipe=g.event.special.swipeleft=g.event.special.swiperight=g.event.special.swipeup=g.event.special.swipedown={setup:function(k,j,i){var k=a(this);
if(k.count++>0){return
}f(this,"moveend",b);
return true
},teardown:function(){var i=a(this);
if(--i.count>0){return
}c(this,"moveend",b);
return true
},settings:e}
});
function lanzarCarrusel(){console.log("lanzarCarrusel"),"function"==typeof $.fn.microfiche?console.log("ya existe la función del plugin microfiche"):(console.log("declaramos plugin icrofiche"),$.fn.microfiche=function(a){return this.each(function(){var b=$(this).data("microfiche");
b?b.run(a):new Microfiche($.extend({el:this},a))
})
})
}!function(){window.Microfiche=function(a){return this.initialize(a),this
},Microfiche.VERSION="1.4.0",$.extend(Microfiche.prototype,{options:{autoplay:!1,autopause:!1,buttons:!0,bullets:!0,cyclic:!1,keyboard:!1,swipe:!0,clickToAdvance:!1,minDuration:250,duration:500,maxDuration:500,dragThreshold:25,elasticity:0.5,swipeThreshold:0.125,prevButtonLabel:"&larr;",nextButtonLabel:"&rarr;"},x:0,initialize:function(a){this.options=$.extend({},this.options,a),this.el=$(a.el),this.el.data("microfiche",this),this.createFilm(),this.createScreen(),this.calibrate(),this.film.width()<=this.screen.width()||(this.createControls(),this.enableTouch(),this.prepareCyclic())
},createFilm:function(){this.film=$('<div class="microfiche-film">').css({position:"absolute"}),this.el.children().appendTo(this.film).css({"float":"left"}),this.prepareFilm&&this.prepareFilm()
...
},prepareCyclic:function(){if(this.options.cyclic){var b=this.film.clone(),a=this.film.clone(),c=this.film.width();
b.prependTo(this.film).css({position:"absolute",left:-c+"px"}),a.appendTo(this.film).css({position:"absolute",left:c+"px"})
}},calibrate:function(){this.screen.width(100000);
var b=this.film.width(),a=this.film.height();
this.film.width(b).height(a),this.screen.width("auto").height(a)
},createControls:function(){var a=this;
this.controls=$('<span class="microfiche-controls" />').appendTo(this.el),this.controls.on("click","a, button.microfiche-button",function(b){a.didClickControl(b)
}),this.options.bullets&&this.createBullets(),this.options.buttons&&this.createButtons(),this.updateControls()
},createBullets:function(){for(var b=$('<span class="microfiche-bullets" />').appendTo(this.controls),a=0;
a<this.totalPageCount();
a++){$("<button>").addClass("microfiche-bullet").attr("data-microfiche-page",a).data("action","slideToPage").data("arguments",[a]).html(a+1).appendTo(b)
}},createButtons:function(){$("<button>").addClass("microfiche-button microfiche-prev-button").attr("rel","prev").attr("title","Atrás").data("action","prev").data("arguments",[]).html(this.options.prevButtonLabel).prependTo(this.controls),$("<button>").addClass("microfiche-button microfiche-next-button").attr("rel","next").attr("title","Siguiente").data("action","next").data("arguments",[]).html(this.options.nextButtonLabel).appendTo(this.controls)
},enableTouch:function(){if(this.options.swipe){var b=this,a=this.touchstart,c=this.touchmove,d=this.touchend;
this.touchstart=function(){a.apply(b,arguments)
},this.touchmove=function(){c.apply(b,arguments)
},this.touchend=function(){d.apply(b,arguments)
},this.film.on("touchstart",this.touchstart)
}},didClickControl:function(b){b.preventDefault();
var a=$(b.target),c=(a.data("action"),a.data("arguments"));
this[a.attr("rel")].apply(this,c)
},touchstart:function(b){var a=b.originalEvent.targetTouches;
!a||a.length>1||(this.touchState={then:new Date,ox:a[0].pageX,oy:a[0].pageY,isDrag:!1},$(document).on("touchmove",this.touchmove).on("touchend",this.touchend))
},touchmove:function(b){var a=b.originalEvent.targetTouches[0],c=a.pageX-this.touchState.ox,d=a.pageY-this.touchState.oy;
if(!this.touchState.isDrag){if(Math.abs(d)>=this.options.dragThreshold){return void this.touchend()
}Math.abs(c)>=this.options.dragThreshold&&(this.touchState.isDrag=!0)
}if(this.touchState.isDrag){b.preventDefault();
var f=new Date,a=f-this.touchState.then;
if(this.touchState.vx=(c-this.touchState.dx)/a,this.touchState.vy=(d-this.touchState.dy)/a,this.touchState.dx=c,this.touchState.dy=d,this.touchState.then=f,this.touchState.cx=this.x-c,!this.options.cyclic){if(this.touchState.cx<this.min()){var g=this.min()-this.touchState.cx;
g*=this.options.elasticity,this.touchState.cx=this.min()-g
}if(this.touchState.cx>this.max()){var g=this.touchState.cx-this.max();
g*=this.options.elasticity,this.touchState.cx=this.max()+g
}}this.film.css({WebkitTransition:"none",WebkitTransform:"translate3d("+-this.touchState.cx+"px, 0px, 0px)"})
}},touchend:function(){if($(document).off("touchmove",this.touchmove).off("touchend",this.touchend),this.touchState.isDrag){var b=this.touchState.dx,a=this.screenWidth(),c=this.touchState.vx,d=this.options.swipeThreshold;
-a*d>=b?this.slideByPages(0.5,c):b>=a*d?this.slideByPages(-1,c):this.slideByPages(0)
}this.screenWidth()>400&&this.screenWidth()<600
},onkeydown:function(a){37!==a.keyCode&&39!==a.keyCode||!this.isCentral("[data-microfiche-keyboard]")||(37===a.keyCode?this.slideByPages(-1):39===a.keyCode&&this.slideByPages(1))
},onmousedown:function(){this.slideByPages(1)
},updateControls:function(){this.options.bullets&&this.updateBullets(),this.options.buttons&&this.updateButtons()
},updateBullets:function(){this.controls.find(".microfiche-bullet").removeClass("selected"),this.controls.find('[data-microfiche-page="'+this.currentPageIndex()+'"]').addClass("selected")
},updateButtons:function(){this.options.cyclic||(this.controls.find('[rel="prev"]').attr("disabled",this.x<=this.min()),this.controls.find('[rel="prev"]').attr("enabled",this.x>this.min()).attr("title","Atrás").mouseover(function(){$(this).css("opacity",".7")
}).mouseout(function(){$(this).css("opacity","1")
}),this.controls.find('[enabled="true"]').css("opacity","1").css("cursor","pointer"),this.controls.find('[rel="next"]').attr("disabled",this.x>=this.max()),this.controls.find('[rel="next"]').attr("enabled",this.x<=this.max()).attr("title","Siguiente").mouseover(function(){$(this).css("opacity",".7")
}).mouseout(function(){$(this).css("opacity","1")
}),this.controls.find('[disabled="disabled"]').css("opacity",".2").css("cursor","default").attr("title",""))
},round:function(b){var a=this.screenWidth();
return Math.round(b/a)*a
},constrain:function(b,a,c){return void 0===a&&(a=this.min()),void 0===c&&(c=this.max()),Math.max(a,Math.min(b,c))
},roundAndConstrain:function(b,a,c){return this.constrain(this.round(b),a,c)
},withinBounds:function(a){return this.min()<=a&&a<=this.max()
...
},isCentral:function(b){var a=$($(b||".microfiche-screen").sort(function(d,c){return Math.abs(1-($(window).scrollTop()+$(window).height()/2-$(d).height()/2)/$(d).offset().top)-Math.abs(1-($(window).scrollTop()+$(window).height()/2-$(c).height()/2)/$(c).offset().top)
})[0]).parent().data("microfiche");
return a===this
},jump:function(){this.el.trigger("microfiche:willMove"),this.performJump(),this.updateControls(),this.el.trigger("microfiche:didMove")
},performJump:function(){this.film.css({left:-this.x})
},transition:function(b){var a=this;
this.options.cyclic&&this.handleWrappingTransition(),null==b&&(b=this.options.duration);
var c=function(){a.afterTransition()
};
this.el.trigger("microfiche:willMove"),setTimeout(function(){a.performTransition(b,c)
})
},handleWrappingTransition:function(){this.x>this.max()?(this.x=this.min()-this.screenWidth(),this.touchState&&this.touchState.dx&&(this.x-=this.touchState.dx),this.jump(),this.x=this.min(),this.updateControls()):this.x<this.min()&&(this.x=this.max()+this.screenWidth(),this.touchState&&this.touchState.dx&&(this.x-=this.touchState.dx),this.jump(),this.x=this.max(),this.updateControls())
},performTransition:function(b,a){this.film.stop().animate({left:-this.x+"px"},b,a)
},afterTransition:function(){delete this.touchState,this.el.trigger("microfiche:didMove")
},slideByPages:function(b,a){var c=this.x,d=this.screenWidth();
if(this.x=this.constrain((Math.round(this.x/d)+b)*d),this.options.cyclic&&this.x==c&&(this.x+=b*d),a){var f=this.constrain(Math.abs((this.x-c)/a),this.options.minDuration,this.options.maxDuration)
}else{var f=this.options.duration
}this.updateControls(),this.transition(f)
},slideToPage:function(a){this.x=this.constrain(a*this.screenWidth()),this.updateControls(),this.transition()
},slideToPoint:function(a){this.x=this.roundAndConstrain(a),this.updateControls(),this.transition()
},jumpToPage:function(a){this.x=this.constrain(a*this.screenWidth()),this.updateControls(),this.jump()
},jumpToPoint:function(a){this.x=this.roundAndConstrain(a),this.updateControls(),this.jump()
},prev:function(){this.slideByPages(-1)
},next:function(){this.slideByPages(1)
}}),"WebKitCSSMatrix" in window&&"m11" in new WebKitCSSMatrix&&$.extend(Microfiche.prototype,{prepareFilm:function(){this.film.css({WebkitTransform:"translate3d(0px, 0px, 0px)"})
},performTransition:function(b,a){this.film.one("webkitTransitionEnd",a).css({WebkitTransition:"-webkit-transform "+b+"ms",WebkitTransform:"translate3d("+-this.x+"px, 0px, 0px)"})
},performJump:function(){this.film.css({WebkitTransition:"-webkit-transform 0ms",WebkitTransform:"translate3d("+-this.x+"px, 0px, 0px)"})
}})
}();
(function(l,f,j,m){console.log("Parallax loaded.");
"use strict";
function c(a,n){this.element=a,this.$context=l(a).data("api",this),this.$layers=this.$context.find(".layer");
var h={calibrateX:this.$context.data("calibrate-x")||null,calibrateY:this.$context.data("calibrate-y")||null,invertX:this.$context.data("invert-x")||null,invertY:this.$context.data("invert-y")||null,limitX:parseFloat(this.$context.data("limit-x"))||null,limitY:parseFloat(this.$context.data("limit-y"))||null,scalarX:parseFloat(this.$context.data("scalar-x"))||null,scalarY:parseFloat(this.$context.data("scalar-y"))||null,frictionX:parseFloat(this.$context.data("friction-x"))||null,frictionY:parseFloat(this.$context.data("friction-y"))||null};
for(var p in h){null===h[p]&&delete h[p]
}l.extend(this,b,n,h),this.calibrationTimer=null,this.calibrationFlag=!0,this.enabled=!1,this.depths=[],this.raf=null,this.ox=0,this.oy=0,this.ow=0,this.oh=0,this.cx=0,this.cy=0,this.ix=0,this.iy=0,this.mx=0,this.my=0,this.vx=0,this.vy=0,this.onMouseMove=this.onMouseMove.bind(this),this.onDeviceOrientation=this.onDeviceOrientation.bind(this),this.onOrientationTimer=this.onOrientationTimer.bind(this),this.onCalibrationTimer=this.onCalibrationTimer.bind(this),this.onAnimationFrame=this.onAnimationFrame.bind(this),this.onWindowResize=this.onWindowResize.bind(this),this.initialise()
}var d="parallax",k=30,b={calibrationThreshold:100,calibrationDelay:500,supportDelay:500,calibrateX:!1,calibrateY:!0,invertX:!0,invertY:!0,limitX:!1,limitY:!1,scalarX:10,scalarY:10,frictionX:0.1,frictionY:0.1};
c.prototype.transformSupport=function(y){for(var q=j.createElement("div"),s=!1,x=null,e=!1,v=null,u=null,i=0,w=this.vendors.length;
w>i;
i++){if(null!==this.vendors[i]?(v=this.vendors[i][0]+"transform",u=this.vendors[i][1]+"Transform"):(v="transform",u="transform"),q.style[u]!==m){s=!0;
break
}}switch(y){case"2D":e=s;
break;
case"3D":s&&(j.body.appendChild(q),q.style[u]="translate3d(1px,1px,1px)",x=f.getComputedStyle(q).getPropertyValue(v),e=x!==m&&x.length>0&&"none"!==x,j.body.removeChild(q))
}return e
},c.prototype.ww=null,c.prototype.wh=null,c.prototype.hw=null,c.prototype.hh=null,c.prototype.portrait=null,c.prototype.desktop=!navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry|BB10|mobi|tablet|opera mini|nexus 7)/i),c.prototype.vendors=[null,["-webkit-","webkit"],["-moz-","Moz"],["-o-","O"],["-ms-","ms"]],c.prototype.motionSupport=!!f.DeviceMotionEvent,c.prototype.orientationSupport=!!f.DeviceOrientationEvent,c.prototype.orientationStatus=0,c.prototype.transform2DSupport=c.prototype.transformSupport("2D"),c.prototype.transform3DSupport=c.prototype.transformSupport("3D"),c.prototype.initialise=function(){"static"===this.$context.css("position")&&this.$context.css({position:"relative"}),this.$layers.css({position:"absolute",display:"block",height:"100%",width:"100%",left:0,top:0}),this.$layers.first().css({position:"relative"}),this.$layers.each(l.proxy(function(a,h){this.depths.push(l(h).data("depth")||0)
...
},c.prototype.updateDimensions=function(){this.ox=this.$context.offset().left,this.oy=this.$context.offset().top,this.ow=this.$context.width(),this.oh=this.$context.height(),this.ww=f.innerWidth,this.wh=f.innerHeight,this.hw=this.ww/2,this.hh=this.wh/2
},c.prototype.queueCalibration=function(a){clearTimeout(this.calibrationTimer),this.calibrationTimer=setTimeout(this.onCalibrationTimer,a)
...
},c.prototype.disable=function(){this.enabled&&(this.enabled=!1,this.orientationSupport?f.removeEventListener("deviceorientation",this.onDeviceOrientation):f.removeEventListener("mousemove",this.onMouseMove),f.removeEventListener("resize",this.onWindowResize),cancelAnimationFrame(this.raf))
},c.prototype.calibrate=function(e,a){this.calibrateX=e===m?this.calibrateX:e,this.calibrateY=a===m?this.calibrateY:a
},c.prototype.invert=function(e,a){this.invertX=e===m?this.invertX:e,this.invertY=a===m?this.invertY:a
},c.prototype.friction=function(e,a){this.frictionX=e===m?this.frictionX:e,this.frictionY=a===m?this.frictionY:a
},c.prototype.scalar=function(e,a){this.scalarX=e===m?this.scalarX:e,this.scalarY=a===m?this.scalarY:a
},c.prototype.limit=function(e,a){this.limitX=e===m?this.limitX:e,this.limitY=a===m?this.limitY:a
},c.prototype.clamp=function(h,a,n){return h=Math.max(h,a),h=Math.min(h,n)
},c.prototype.css=function(p,s,t){for(var u=null,h=0,q=this.vendors.length;
q>h;
h++){if(u=null!==this.vendors[h]?l.camelCase(this.vendors[h][1]+"-"+s):s,p.style[u]!==m){p.style[u]=t;
break
}}},c.prototype.accelerate=function(h){for(var a=0,o=h.length;
o>a;
a++){var n=h[a];
this.css(n,"transform","translate3d(0,0,0)"),this.css(n,"transform-style","preserve-3d"),this.css(n,"backface-visibility","hidden")
}},c.prototype.setPosition=function(h,a,n){a+="%",n+="%",this.transform3DSupport?this.css(h,"transform","translate3d("+a+","+n+",0)"):this.transform2DSupport?this.css(h,"transform","translate("+a+","+n+")"):(h.style.left=a,h.style.top=n)
},c.prototype.onOrientationTimer=function(){this.orientationSupport&&0===this.orientationStatus&&(this.disable(),this.orientationSupport=!1,this.enable())
},c.prototype.onCalibrationTimer=function(){this.calibrationFlag=!0
},c.prototype.onWindowResize=function(){this.updateDimensions()
},c.prototype.onAnimationFrame=function(){var q=this.ix-this.cx,p=this.iy-this.cy;
(Math.abs(q)>this.calibrationThreshold||Math.abs(p)>this.calibrationThreshold)&&this.queueCalibration(0),this.portrait?(this.mx=(this.calibrateX?p:this.iy)*this.scalarX,this.my=(this.calibrateY?q:this.ix)*this.scalarY):(this.mx=(this.calibrateX?q:this.ix)*this.scalarX,this.my=(this.calibrateY?p:this.iy)*this.scalarY),isNaN(parseFloat(this.limitX))||(this.mx=this.clamp(this.mx,-this.limitX,this.limitX)),isNaN(parseFloat(this.limitY))||(this.my=this.clamp(this.my,-this.limitY,this.limitY)),this.vx+=(this.mx-this.vx)*this.frictionX,this.vy+=(this.my-this.vy)*this.frictionY;
for(var w=0,u=this.$layers.length;
u>w;
w++){var x=this.depths[w],y=this.$layers[w],h=this.vx*x*(this.invertX?-1:1),v=this.vy*x*(this.invertY?-1:1);
this.setPosition(y,h,v)
}this.raf=requestAnimationFrame(this.onAnimationFrame)
},c.prototype.onDeviceOrientation=function(a){if(!this.desktop&&null!==a.beta&&null!==a.gamma){this.orientationStatus=1;
var i=(a.beta||0)/k,h=(a.gamma||0)/k,n=f.innerHeight>f.innerWidth;
...
}},c.prototype.onMouseMove=function(a){this.ix=(a.pageX-this.hw)/this.hw,this.iy=(a.pageY-this.hh)/this.hh
};
var g={enable:c.prototype.enable,disable:c.prototype.disable,calibrate:c.prototype.calibrate,friction:c.prototype.friction,invert:c.prototype.invert,scalar:c.prototype.scalar,limit:c.prototype.limit};
l.fn[d]=function(a){var h=arguments;
return this.each(function(){var i=l(this),e=i.data(d);
e||(e=new c(this,a),i.data(d,e)),g[a]&&e[a].apply(e,Array.prototype.slice.call(h,1))
})
}
})(window.jQuery||window.Zepto,window,document),function(){for(var b=0,a=["ms","moz","webkit","o"],c=0;
a.length>c&&!window.requestAnimationFrame;
...
}window.requestAnimationFrame||(window.requestAnimationFrame=function(d){var g=(new Date).getTime(),f=Math.max(0,16-(g-b)),h=window.setTimeout(function(){d(g+f)
},f);
return b=g+f,h
}),window.cancelAnimationFrame||(window.cancelAnimationFrame=function(d){clearTimeout(d)
})
}();