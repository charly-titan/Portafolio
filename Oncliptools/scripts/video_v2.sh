...
source $(dirname $0)/video_config.sh
...
case "$SIGNAL" in
		...
		Quantel) echo "Canal Quantel"
				...
				declare -A BITRATE=( ["150"]="214000" ["480"]="544000" ["600"]="664000" )
				MASTERQUALITY=1034000
				MAXQUALITY=600
			;;
...
echo "##**# mkdir -p $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME ##**#"
...
echo "[[ E:CREANDO DIRECTORIO:$(date +%s) ]]"
 ...
 if [ $VIDEO_TYPE = "SHORT" ]
	then
		if [ -f "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4" ];
		then
			echo "File $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4 exists."
			echo "<!-- FILE_EXIST -->"
   			exit 0
		else
			echo "File $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4 does not exist."
			echo "##**# mono $HLS_PROGRAM -s $DVRSTREAMURL --ignore 'a-p' --ignore 'a-b' -b 299-400 -v --start $INITIAL_POINT --end $END_POINT -o $OUTPUT_DIRECTORY$FILE_NAME ##**#"
 			mono "$HLS_PROGRAM" -s "$DVRSTREAMURL" --ignore 'a-p' --ignore 'a-b' -b 299-400 -v --start "$INITIAL_POINT" --end "$END_POINT" -o "$OUTPUT_DIRECTORY$FILE_NAME"
		fi
 	else
 ...
 if [ $VIDEO_TYPE = "SHORT" ]
 	then
 		...
 		then
 			echo "##**# mv $OUTPUT_DIRECTORY$FILE_NAME"_350000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4 ##**#"
 			mv "$OUTPUT_DIRECTORY$FILE_NAME"_350000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4"
 		elif [ -f "$OUTPUT_DIRECTORY$FILE_NAME"_299000.mp4 ];
 		then
 			echo "##**# mv $OUTPUT_DIRECTORY$FILE_NAME"_299000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4 ##**#"
 			mv "$OUTPUT_DIRECTORY$FILE_NAME"_299000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4"
 		else
 			echo "##**# mv $OUTPUT_DIRECTORY$FILE_NAME"_354000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4 ##**#"
 			mv "$OUTPUT_DIRECTORY$FILE_NAME"_354000.mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4"
 		fi
 	else
 		for QUALITY in "${!BITRATE[@]}"
		do 
 		echo "##**# mv $OUTPUT_DIRECTORY$FILE_NAME"_"${BITRATE["$QUALITY"]}".mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4 ##**#"
 		mv "$OUTPUT_DIRECTORY$FILE_NAME"_"${BITRATE["$QUALITY"]}".mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4" 
 		done
 	fi
...
if [ $VIDEO_TYPE = "SHORT" ]
	then
		...
		echo "##**# $FFMPEG -async 2 -i $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4 -g 2 $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4 ##**#"
		$FFMPEG -async 2 -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4" -g 2 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4"
		echo "[[ E:SINCRONIZANDO AUDIO Y VIDEO:$(date +%s) ]]"		
	else
		echo "[[ S:CAMBIANDO EL KEYINTERVAL:$(date +%s) ]]"
		for QUALITY in "${!BITRATE[@]}"
		do
			echo "##**# $FFMPEG -i $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4 -g 2 -vcodec copy -acodec copy $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4 ##**#"
			$FFMPEG -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4" -g 2 -vcodec copy -acodec copy "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4"
		done
...
if [ $VIDEO_TYPE = "FULL" ]
	then
		...
		for QUALITY in "${!BITRATE[@]}"
		do
		echo "##**# rm $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4 ##**#"
		rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip-g.mp4"
		done
		...
		for QUALITY in "${!BITRATE[@]}"
		do
			echo "##**# $FFMPEG -async 2 -i $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4 -g 2 -ss $SECONDS_START -t $SECONDS_END $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY.mp4 ##**#"
			$FFMPEG -async 2 -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4" -g 2 -ss "$SECONDS_START" -t "$SECONDS_END" "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY.mp4"
		done
		...
		for QUALITY in "${!BITRATE[@]}"
		do
		echo "##**# rm $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4 ##**#"
		rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$QUALITY-lip.mp4"
		done
		echo "[[ S:ELIMINANDO RESIDUOS LIPSYNC:$(date +%s) ]]"

		echo "[[ S:GENERANDO THUMBNAILS:$(date +%s) ]]"
		echo "##**# $FFMPEG" -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$MAXQUALITY.mp4" -vcodec mjpeg -vframes 1 -an -f rawvideo -s 160x90 -ss 00:00:10 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME.jpg ##**#"
		$FFMPEG -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$MAXQUALITY.mp4" -vcodec mjpeg -vframes 1 -an -f rawvideo -s 160x90 -ss 00:00:10 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME.jpg"
		echo "##**# $FFMPEG" -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$MAXQUALITY.mp4" -vcodec mjpeg -vframes 1 -an -f rawvideo -s 854x480 -ss 00:00:10 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-STILL.jpg ##**#"
		$FFMPEG -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-$MAXQUALITY.mp4" -vcodec mjpeg -vframes 1 -an -f rawvideo -s 854x480 -ss 00:00:10 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-STILL.jpg"
		echo "[[ E:GENERANDO THUMBNAILS:$(date +%s) ]]"

		echo "[[ S:SUBIENDO A AKAMAI:$(date +%s) ]]"
		echo "##**# ncftpput -u uptvvid -p zHJX1jXA.. -v -R -B 6291456 cmflash.upload.akamai.com /tst/ $DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME ##**#"
		ncftpput -u uptvvid -p zHJX1jXA.. -v -R -B 6291456 cmflash.upload.akamai.com /tst/ "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME"
		echo "[[ E:SUBIENDO A AKAMAI:$(date +%s) ]]"

		echo "[[ S:GENERANDO MASTER:$(date +%s) ]]"
		# mv "$OUTPUT_DIRECTORY$FILE_NAME"_"$MASTERQUALITY".mp4 "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip-g.mp4"
		# $FFMPEG -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip-g.mp4" -g 2 -vcodec copy -acodec copy "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip.mp4"
		# rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip-g.mp4"
		# $FFMPEG -async 2 -i "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip.mp4" -g 2 -ss "$SECONDS_START" -t "$SECONDS_END" "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000.mp4"
		# rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000-lip.mp4"
		# mv "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000.mp4" "$DIRECTORY_MASTER$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-4000.mp4" 
		echo "[[ S:GENERANDO MASTER:$(date +%s) ]]"
		...
		echo "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-150.mp4"
		SIZE_150=$(cat "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-150.mp4" | wc -c | tr -d ' ' )
		SIZE_480=$(cat "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-480.mp4" | wc -c | tr -d ' ' )
		SIZE_600=$(cat "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-600.mp4" | wc -c | tr -d ' ' )
		SIZE_THB=$(cat "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME.jpg" | wc -c | tr -d ' ' )
		SIZE_STL=$(cat "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-STILL.jpg" | wc -c | tr -d ' ' )
...
if [ $VIDEO_TYPE = "SHORT" ]
	then
		...
		cd $MOOV_DIRECTORY
		$PHP_EXEC "$MOOV_PHP" "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4"
		echo "[[ E:REALIZANDO FIX A METADATA DEL VIDEO:$(date +%s) ]]"

		echo "[[ S:ELIMINANDO INFORMACION TEMPORAL:$(date +%s) ]]"
		rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-orig.mp4"
		rm "$DIRECTORY_MEDIA$VIDEO_TYPE/$PROGRAM_NAME/$DIRECTORY_NAME/$FILE_NAME-235-edit.mp4"
