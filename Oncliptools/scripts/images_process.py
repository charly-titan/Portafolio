...
#se obtiene el tiempo del que se generan las imagenes, que sera 1 min atras
actual_time=time.strftime("%d/%m/%Y %H:%M")
make_time=int(time.mktime(datetime.datetime.strptime(actual_time,"%d/%m/%Y %H:%M").timetuple())-60)
if (oncliptools.debug): #************Solo para pruebas locales***********#
        make_time=1424830800 
try:
