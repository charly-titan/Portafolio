...
<div class="row">
    <div class="col-md-8">
        <section class="widget">
            <header>
                <h4>Video<small> Canal {{$info['channel']}}</small></h4>
            </header>
            ...
            <div id="boxSlider">
                <div class="body text-align-center well">
                    ...
                    <div id="visits-chart" class="row">
                        ...
                        <br><br>
                        <div>
                            <div style="width:100%" class="js-slider" id="slider-0" data-slider-value="[0,{{$info['time']}}]"></div>
                        </div>
                    </div>
                ...
                <div id="options" class="tab-pane active clearfix">
                    <fieldset>
                        ...
                        <div class="controls form-group">
                            ...
                            <label class="checkbox">
                                <input name="cq5deportesCheck" id="cq5deportesCheck" type="checkbox" class="check" checked>
                                {{Lang::get('vcms.CQ5deportes_label')}}
                            </label>
                            <div id="ramaCQ5deportes" class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="time" id="time" value="{{round($info['time']/60,1)}}">
                            <input type="hidden" name="canal" id="canal" value="{{$info['channel']}}">
                            <input type="hidden" name="startDateCal" id="startDateCal" value="{{$info['startDateCal']}}">
                            <input type="hidden" name="startTime" id="startTime" value="{{$info['startTime']}}">
                            <input type="hidden" name="FINI" id="FINI" value="{{$info['start']}}">
                            <input type="hidden" name="FFIN" id="FFIN" value="{{$info['end']}}">
                        </div>
                        <div class="control-group">
 ...
 @parent
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            for( var i = 0; i < ramasArray.length; i++ ){
                ...
                if((ramasArray[i][1]=="1363")||(rama.substring(0, 4)=="1363")){
                    ...
                    if(ramasArray[i][0]=="only_mex")
                        $("#nodeGalaxy").append('<option selected value="' + ramasArray[i][1] + '">' + ramasArray[i][0] + '</option>');
                    else    
            ...
            /*valores predeterminados para deportes*/
            $('#nodeCQ5deportes > option[value="13630203"]').attr('selected', 'selected');
            $('#nodeCQ5deportes').select2();
            $('#program > option[value="futbo"]').attr('selected', 'selected');
            $('#program').select2();
            /**************************************/
            ...
            $("#slider-0").slider('setValue',[30,time-30]);
            ...
            parametros = {
                "canal" : "{{$info['channel']}}",
                "startDateCal": "{{$info['startDateCal']}}",
                "startTime": "{{$info['startTime']}}",
                "time": "{{round($info['time']/60,1)}}",
                "FINI": "{{$info['start']}}",
                "FFIN": "{{$info['end']}}"
            }; 
            updateThumb();
            /*$.ajax({
                ...
                success: function(result) {
                    console.log(result);
                    info_down=result['infoDown'];
                    intervalDown = setInterval(downSignalThumb,10000);
                },
            ...
            $("#boxSlider").on("slideStop",".js-slider", function(slideEvt) {
                ...
                if (start_global[cut]==start){
                    ...
                    try{
                        posThumb=parseInt((valores[1]-3)/10);
                    ...
                    try{
                        posThumb=parseInt((valores[0])/10);
                        carousel.setCurrent( posThumb );
                    }catch(e){}      
            ...
            /*Controla play y pause del video
        ...
        function loadHandler(event) {
            var config_overrides = {
                ...
                media:{
                    title: "Live",
                    //temporalType: "live",
                    //start_bitrate_index: "10",
                    //rewind_interval: "30",
                    //start: 10,
                    source:[
                            {src: "<?=$info['src']?>" , type: "video/mp4"}
                    ]
                },
                captioning:{
                    enabled: false
            ...
            amp.addEventListener("started", adStartedHandler);
            //amp.addEventListener("ended", adEndedHandler);
        ...
        function timeUpdateHandler(event) {
            ...
            timeAct=amp.getCurrentTime().toFixed(1);
            document.getElementById(id).innerHTML = timeAct;
            ...
            if( timeAct == parseFloat($("#tend-"+actual_id).val()) ){
                amp.pause();
                $("#pause-"+actual_id).hide();
                $("#play-"+actual_id).show();
            }else{
        ...
        /*function downSignalThumb(){
            $.ajax({
                    ...
                    success: function(result) {
                        console.log(result);
                        if (typeof result['infoImg'] != 'undefined') {
                            clearInterval(intervalDown); // stop the intervalDown
                            info_img=result['infoImg'];
                            interval = setInterval(updateThumb,10000);
                        }
                    },
                    error: function(result) {
                        clearInterval(intervalDown); // stop the intervalDown
                        ...
                        if(imgThumb.length > 0){
                            ...
                            for(thumb in imgThumb){
                                imgUrl=imgThumb[thumb]['url'];
                                    $('#carousel').append('<li><a href="#"><img src="'+imgThumb[thumb]['url']+'" alt="'+imgThumb[thumb]['name']+'" /></a></li>');
                            }
