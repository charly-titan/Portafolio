...
@section('content')
    ...
    @if (isset($info->properties['colorFont']))
        .main-form h2{color: {{$info->properties['colorFont']}}; }
    @endif
    @if(isset($info->properties['colorHeader']))
    .main-form .btn-confirmar{background-color:{{$info->properties['colorHeader']}}; }
    .main-form .btn{background:{{$info->properties['colorHeader']}}; }
    .pika-single .pika-label:first-child:after, .pika-single .pika-label:nth-child(2):after{background:{{$info->properties['colorHeader']}};}
    .pika-single .is-selected .pika-button{background:{{$info->properties['colorHeader']}};}
    @endif
    {{isset($info->properties['colorHeader'])?'.btn-confirma{background:'.$info->properties['colorHeader'].'!important;}':''}}
    </style>
	<article class="left-container">
        <div class="main-form">
            <h2>{{Lang::get('promociones.formTitle')}}</h2>
                {{Form::open(array('url'=>'/'.(isset($info->properties['channel'])?$info->properties['channel']:'').'/'.$short_name."/confirm-data",'id'=>'contact-form-confirm','method' => 'post'))}}
                    ...
                    <div class="form-box-50">
                        ...
                        <span class="span"></span>
                        {{Form::text('date',(!is_null(Session::get("user.birthyear")) && Session::get("user.birthyear")!="" && Session::get("user.birthyear")!="0" && Session::get("user.birthyear")!=0 )?sprintf("%02d",Session::get("user.birthday"))."/".sprintf("%02d",Session::get("user.birthmonth"))."/".Session::get("user.birthyear"):Lang::get('promociones.formDate'),array('placeholder'=>Lang::get('promociones.formDate'),'data-requiere'=>'true','data-format'=>'date','data-invalid'=>Lang::get('promociones.invalidDate'),'data-null'=>Lang::get('promociones.formDateMsgNull')))}}
                    ...
                    <div class="form-box-100 check">
                        ...
                        {{ HTML::link('/'.(isset($info->properties['channel'])?$info->properties['channel']:'').'/'.$short_name.'/bases-concurso', Lang::get('promociones.formBaseCond'),array("target"=>"_blank")) }}
                        <p class="span1"></p></div>
                        @endif
                        <div class="checkbox">
                        ...
                        {{ HTML::link('/'.(isset($info->properties['channel'])?$info->properties['channel']:'').'/'.$short_name.'/aviso-privacidad', Lang::get('promociones.formAvsPriv'),array("target"=>"_blank")) }}
                        <p class="span2"></p></div>
                    </div>
