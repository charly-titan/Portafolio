...
<head>
    ...
    <link href="/amqlichita/css/global.css" rel="stylesheet" />
    <!-- Modenizer -->
...
<body style="background-color: #16d5e8">
    ...
    <br />
    <div class="container">
        <div class="full-video login-ligth">
            <div class="cont-full slideDownVideo">
                <div class="login-form">
                    <h6>Confirma tus datos</h6>
                    {{Form::open(array('url'=>'/amqlichita/1/confirma','id'=>'contact-form-confirm','method' => 'post'))}}
                        @if (Session::get('user.provider')=="Twitter")
                            <label for="nombre">Nombre(s)</label>
                            <input type="text" name="usrname" required placeholder="Tu nombre" autocomplete="off">
                            {{ $errors->first('usrname', '<div class="error">:message</div>') }}
                            <label for="apellido">Apellido(s)</label>
                            <input type="text" name="lastname" required placeholder="Tu apellido" autocomplete="off">
                            {{ $errors->first('lastname', '<div class="error">:message</div>') }}
                            <label for="email">Email</label>
                            <input type="email" name="email" required placeholder="ejemplo@televisa.com" autocomplete="off">
                            {{ $errors->first('email', '<div class="error">:message</div>') }}
                        @endif
                        <label for="date">Fecha de nacimiento</label>
                        <div class="pickaday"></div>
                        <input type="date" name="date" required placeholder="dd/mm/aaaa" id="date" autocomplete="off">
                        {{ $errors->first('date', '<div class="error">:message</div>') }}
                        @if (Session::get('user.provider')=="Twitter")
                        <label for="genero" class="genero">Sexo</label>
                        <input type="radio" name="genero" value="male" required=""><span class="lbl">Hombre</span>
                        <input type="radio" name="genero" value="female" required=""><span class="lbl">Mujer</span>
                        @endif
                        <div>
    ...
    <!--script type="text/javascript">
        var tagdate = $('#date');
        if (tagdate.length) {
            head.js('/amqlichita/js/jquery.datepick.js',function(){
                function putwidth(){
                    $('.datepick').width(tagdate.outerWidth());
                }
                if($('html').hasClass('desktop')){
                    tagdate.attr('type', 'text');
                    tagdate.datepick({
