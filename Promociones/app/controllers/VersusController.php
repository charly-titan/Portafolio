...
class VersusController extends BaseController {
		...
		if ($short_name=="") {
			...
			if($this->contestExist($short_name)){
				...
				if(isset($info->properties['gigyaOption']) && $info->properties['gigyaOption']=='1'){
					...
					//"Login research";
					return View::make(Config::get( 'app.main_template' ).".".$this->vista.'.versus')->with(array('questionAll'=>$this->questionAll($id_question,$short_name),'info'=>$info, 'UsersTop'=>$UsersTop, 'point'=>$point, 'category'=>$category, 'adUnit'=>'versus', 'vista'=>$this->vista));
				}	
