...
return calendar;},defaults={field:null,btn:"c5-calendario",bound:undefined,position:'bottom left',reposition:true,format:'YYYY-MM-DD',defaultDate:null,setDefaultDate:false,firstDay:0,minDate:null,maxDate:null,yearRange:10,showWeekNumber:false,minYear:1950,maxYear:9999,minMonth:undefined,maxMonth:undefined,isRTL:false,yearSuffix:'',showMonthAfterYear:false,numberOfMonths:1,mainCalendar:'left',container:undefined,i18n:{previousMonth:'Mes Anteror',nextMonth:'Mes Siguiente',months:['ENERO','FEBRERO','MARZO','ABRIL','MAYO','JUNIO','JULIO','AGOSTO','SEPTIEMBRE','OCTUBRE','NOVIEMBRE','DICIEMBRE'],weekdays:['Domingo','Lunes','Martes','Miercoles','Jueves','Viernes','Sabado'],weekdaysShort:['Do','Lu','Ma','Mi','Ju','Vi','Sa']},onSelect:null,onOpen:null,onClose:null,onDraw:null},renderDayName=function(opts,day,abbr)
...
for(arr=[];i<j&&i<=opts.maxYear;i++){arr.push('<option value="'+i+'"'+(i===year?' selected':'')+'>'+(i)+'</option>');}
...
if(c===0){html+='<button class="pika-prev c5-izquierda'+(prev?'':' is-disabled')+'" type="button">'+opts.i18n.previousMonth+'</button>';}
if(c===(instance._o.numberOfMonths-1)){html+='<button class="pika-next c5-derecha'+(next?'':' is-disabled')+'" type="button">'+opts.i18n.nextMonth+'</button>';}
...
{var self=this,opts=self.config(options),_targetGlobal=0;self._onMouseDown=function(e)
{if(!self._v){return;}
...
{e=e||window.event;var target=e.target||e.srcElement;if(!target){return;}
if(hasClass(target,'pika-select-month')){self.gotoMonth(target.value);Pikaday._targetGlobal=target;}
else if(hasClass(target,'pika-select-year')){Pikaday._targetGlobal=target;self.gotoYear(target.value);}};self._onInputChange=function(e)
...
{if(self.isVisible()){self.hide();}else{self.show();}};self._onInputClick=function()
{if(self.isVisible()){self.hide();}else{self.show();}};self._onInputBlur=function()
{var pEl=document.activeElement;do{if(hasClass(pEl,'pika-single')){return;}}
while((pEl=pEl.parentNode));if(!self._c){self._b=sto(function(){},50);}
self._c=false;};self._onClick=function(e)
{e=e||window.event;var target=e.target||e.srcElement,pEl=target;if(!target){return;}
if(!hasEventListeners&&hasClass(target,'pika-select')){if(!target.onchange){target.setAttribute('onchange','return;');addEvent(target,'change',self._onChange);}}
do{if(hasClass(pEl,'pika-single')||pEl===opts.trigger){return;}}
while((pEl=pEl.parentNode));if(self._v&&target!==opts.trigger&&target.className!==opts.triggerBtn&&pEl!==opts.trigger){self.hide();}};self.el=document.createElement('div');self.el.className='pika-single'+(opts.isRTL?' is-rtl':'');addEvent(self.el,'mousedown',self._onMouseDown,true);addEvent(self.el,'change',self._onChange);if(opts.field){if(opts.container){opts.container.appendChild(self.el);}else if(opts.bound){document.body.appendChild(self.el);}else{opts.field.parentNode.insertBefore(self.el,opts.field.nextSibling);}
addEvent(opts.field,'change',self._onInputChange);if(!opts.defaultDate){if(hasMoment&&opts.field.value){opts.defaultDate=moment(opts.field.value,opts.format).toDate();}else{opts.defaultDate=new Date(Date.parse(opts.field.value));}
opts.setDefaultDate=true;}}
var defDate=opts.defaultDate;if(isDate(defDate)){self.gotoDate(defDate);}else{self.gotoDate(new Date());}
if(opts.bound){this.hide();self.el.className+=' is-bound';addEvent(opts.trigger,'click',self._onInputClick);addEvent(opts.trigger,'blur',self._onInputBlur);}else{this.show();}};Pikaday.prototype={config:function(options)
{if(!this._o){this._o=extend({},defaults,true);}
var opts=extend(this._o,options,true);opts.isRTL=!!opts.isRTL;opts.field=(opts.field&&opts.field.nodeName)?opts.field:null;opts.bound=!!(opts.bound!==undefined?opts.field&&opts.bound:opts.field);opts.trigger=(opts.trigger&&opts.trigger.nodeName)?opts.trigger:opts.field;opts.triggerBtn=(opts.triggerBtn&&opts.triggerBtn.nodeName)?opts.triggerBtn:opts.btn;var nom=parseInt(opts.numberOfMonths,10)||1;opts.numberOfMonths=nom>4?4:nom;if(!isDate(opts.minDate)){opts.minDate=false;}
...
this.el.innerHTML=html;var f=$('.pika-title .pika-label').eq(0).find('select option:selected').text();var z=(typeof Pikaday._targetGlobal!="undefined")?f:'MES';if($('#date').length){var Mes_d=$('.pika-title .pika-label').eq(0).find('select')[0].outerHTML,Ano_d=$('.pika-title .pika-label').eq(1).find('select')[0].outerHTML;if($('#date').hasClass('firstClick')){$('#date').removeClass('firstClick');$('.pika-title .pika-label').eq(0).html('MES'+Mes_d);$('.pika-title .pika-label').eq(1).html('A&#209;O'+Ano_d);}}
...
top=top+9;this.el.style.cssText=['position: absolute','left: '+left+'px','top: '+top+'px'].join(';');},render:function(year,month)
...
{if($('#date').val()===''){$('#date').addClass('firstClick');}
...
{this.hide();removeEvent(this.el,'mousedown',this._onMouseDown,true);removeEvent(this.el,'change',this._onChange);if(this._o.field){removeEvent(this._o.field,'change',this._onInputChange);if(this._o.bound){removeEvent(this._o.trigger,'click',this._onInputClick);removeEvent(this._o.trigger,'blur',this._onInputBlur);}}
