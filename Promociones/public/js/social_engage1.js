...
if(document.location.href.search("television.televisa.com")!=-1){
	...
	if(document.location.href.search("antes-muerta-que-lichita")!=-1){
		var social_engage_external_config ={
	        ...
	        templates       : {
	            usernotlogged   :   '<p id="start_session"><i class="icon-user"></i><span>Iniciar sesión</span></p>',
	            userlogged      :   '<img id="close_session" src="{{thumbnailURL}}" width="48" height="48"/>',

	            social_networks  :   '<h6>Ingrese a su cuenta</h6>'+
	                                '<div id="gigya_modal" class="social-component"></div>',

	            login_first     :  '<h6>Gracias por registrarte</h6>'+
	    ...
	    function closeModal(){
	        //console.log("callback: OnCloseModal");
	    }

	    function modalStart(){
	        //console.log("callback: OnStartModal");
	    } 

	    function isLogged(){
	        //console.log("callback user logged--  uid:" +uid);
	    }
	    function isNewUser(){
	        console.info("callback: newUser");
	        $("#gigya_body_networks").html(social_engage.templates.login_first);
	        $("#login_gigya_social").show();
	        $("#gigya_body_networks").css("display","block");
        ...
        templates       : {
            ...
            closeSession	: 	'<div class="ui-gy-modal__content alert js-config">'+
									...
									'<button id="close_session" class="ui-gy-btn ui-gy-btn--block ui-gy--tele">Cerrar Sesión</button>'+
...
<<<<<<< HEAD
	    ...
	    function prepareUI(){
	        ...
	        if(document.getElementById("social_content_gigya") != "undefined"){ $("#social_content_gigya").remove(); }
	        $('<section id="social_content_gigya" class="ui-gy-modal" data-animate="fade-in"></section>').appendTo(document.body);
	        $("#social_content_gigya").hide();
	        
	    }

	    function closeModal(){
	        $("#social_content_gigya").hide();
	        $("#social_content_gigya").html(social_engage.templates.closeSession);
	        $(".ui-gy-modal-icon--close").click(function(){
	        	$("#social_content_gigya").hide();
	        });
	        $("#usr_account").click(function(){
	        	$("#social_content_gigya").show();
	        });
	        $("#close_session").click(function(){
	        	social_engage.destroySesion();
	        	$(social_engage.templates.modalstructure).appendTo(document.body);
	        	//social_engage.createCustomModal();
	        });
	    }

	    function modalStart(){
	        $("#login_gigya_social").appendTo("#social_content_gigya");
	        $("#social_content_gigya").show();
	    ...
	    function isLogged(){
	        $("body").addClass("ui-gy");
	        if($(window).width() >= 648){
	        	if(document.getElementById("login_gigya_social")){ $("#login_gigya_social").remove();}
	        	$("#login_gigya_mobile").html("");
	        	$("#login_gigya").css("display","inline-block");
	        }else if($(window).width() <= 647){
	        	$("#login_gigya").html("");
...
>>>>>>> 425d9e427db1e86ed837bc367122eca0355a6ab5
	...
	templates   	: {
	    ...
	    user_status 	: 	'<div class="login-form">'+
                            	...
                            	'<button id="close_session" class="close_gigya"><span>Cerrar Sesión</span></button>'+
                      		'</div>',

		userEmail		:   '<div id="social_gy_mail" class="social-gy-mail-form">'+
								'<p></p>'+
								'<h4 class="gy-txt-subtitle">Ingresa tu correo electrónico</h4>'+
